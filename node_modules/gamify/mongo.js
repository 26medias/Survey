...
function mongoInstance(Gamify) {
	...
	mongo.prototype.init = function(callback) {
		var scope 		= this;
		if (Gamify.settings.mongo.remote) {
			//Gamify.log("Using remote Mongo DB.",Gamify.settings.mongo.server);
			this.server 	= new mongodb.Server(Gamify.settings.mongo.server, Gamify.settings.mongo.port, {auto_reconnect:true});
			this.db			= new mongodb.Db(this.options.database, this.server, {w:1, safe:true});
			this.db.open(function (error, client) {
				if (error) {
					throw error;
				}
				scope.db.authenticate(Gamify.settings.mongo.login, Gamify.settings.mongo.password, function(autherr, result) {
			...
			//Gamify.log("Using local Mongo DB.",this.options.host);
	...
	mongo.prototype.update = function(options, callback) {
		...
		this.open(options.collection, function(collection) {
			collection.update(options.query,options.data,options.options, function(err, res) {
				if (options.return) {
					scope.find({
...
exports.main = function(Gamify) {
	return mongoInstance(Gamify);
};