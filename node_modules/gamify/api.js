...
api.prototype.init = function(app, callback) {
	var scope 			= this;
	this.endpoints 		= {};
	this.authmethods 	= {};
	this.pages 			= {};
	...
	scanStack.add(function(params, onProcessed) {
		file.listFiles("./pages/controllers","js", function(pages) {
			var i;
			var m;
			...
			for (i=0;i<pages.length;i++) {
				var includepath				= pages[i].substr(0,pages[i].length-3);
				var	parts					= includepath.split("/");
				var urlpath					= parts[parts.length-1];
				var apiClass 				= require('../../'+includepath).page;
				subStack.add(function(params, onProcessed) {
					(new params.apiClass()).init(scope.Gamify, function(paths) {
						scope.pages[params.urlpath] 	= {};
						console.log("++ "+params.urlpath);
						for (m in paths) {
							console.log(
								(paths[m].auth?'\033[31m':'\033[32m')+
								scope.Gamify.utils.pad(
									m,
								24)+
								(paths[m].auth ? '\033[35m'+
								scope.Gamify.utils.pad(
									'['+paths[m].auth+']',
								16)+ '\033[31m' : scope.Gamify.utils.pad("",16)+'\033[32m')+
								(paths[m].require.length==0?'':" \033[33m["+paths[m].require.join(', ')+"]")+'\033[0m');
							scope.pages[params.urlpath][m] = paths[m];
						}
						onProcessed();
					});
	...
	scanStack.process(function() {
		// Setup the API routes
		scope.Gamify.log("pages", scope.pages);
		var group;
		var path;
		for (group in scope.pages) {
			for (path in scope.pages[group]) {
				(function(path) {
					scope.app.get(path, function(req, res){
						var data = {};
						...
						scope.Gamify.api.executePage(group, path, data, function() {}, req, res);
					});
					scope.app.post(path, function(req, res){
						var data = {};
						if (req.route.method=='post') {
							data = _.extend({}, req.body);
						} else {
...
api.prototype.executePage = function(group, path, data, callback, req, res) {
	...
	if (!this.pages[group]) {
		...
		errorResponse 	= this.errorResponse("No such group.");
		error			= true;
	}
	if (this.pages[group] && !this.pages[group][path]) {
		console.log("No such path: ",group+'.'+path);
		// Create an error response
		errorResponse 	= this.errorResponse("No such path: ",group+'.'+path);
		error			= true;
	}
	...
	if (!error) {
		...
		var apiMethod = function(params) {
			...
			scope.pages[group][path].callback(params, req, res, function(response) {
			
				// Do we have hooks?
				scope.Gamify.hook.trigger(group, path, data, response, function(modifiedResponse) {
					if (res) {
		...
		var hasParameters = scope.Gamify.api.require(data, scope.pages[group][path].require);
		if (hasParameters !== true) {
			if (res) {
				scope.outputs[format](hasParameters, res, data);
			}
			if (callback) {
		...
		// Is that an auth API?
		if (!scope.pages[group][path].auth) {
			// If an authtoken is given anyway
			if (data.authtoken) {
				// Try to auth the user
				scope.authmethods.authtoken(res, req, data, function(valid, response) {
					if (valid !== false) {
						data.__auth 		= valid;
						data.__authcheck 	= scope.Gamify.settings.systoken;
					}
					// call the method
					apiMethod(data);
				});
			...
			if (!scope.authmethods[scope.pages[group][path].auth]) {
				console.log("No such auh method: ",scope.pages[group][path].auth);
				if (res) {
					scope.outputs[format](scope.errorResponse("No such auh method: "+scope.pages[group][path].auth), res, data);
				}
				if (callback) {
					callback(scope.errorResponse("No such auh method: "+scope.pages[group][path].auth));
				}
			} else {
				//console.log("executing \033[32m"+endpoint+"."+method+"\033[0m with parameters:",modifiedData);
				scope.authmethods[scope.pages[group][path].auth](res, req, data, function(valid, response) {
					...
					if (valid !== false) {
						...
						if (res) {
							scope.outputs[format](_.extend({invalid_token:true},scope.errorResponse(response)), res, data);
						}
						if (callback) {
		...
		scope.Gamify.validator.trigger(endpoint, method, data, function(modifiedData) {
			...
			if (!scope.endpoints[endpoint][method].auth) {
				...
				if (modifiedData.authtoken) {
					// Try to auth the user
					scope.authmethods.authtoken(res, req, modifiedData, function(valid, response) {
						if (valid !== false) {
				...
				if (!scope.authmethods[scope.endpoints[endpoint][method].auth]) {
					...
					//console.log("executing \033[32m"+endpoint+"."+method+"\033[0m with parameters:",modifiedData);
					scope.authmethods[scope.endpoints[endpoint][method].auth](res, req, modifiedData, function(valid, response) {
