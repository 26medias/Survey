...
app.use(flash());
app.use(serveFavicon(__dirname + '/pages/images/favicon.ico'));
//app.use(morgan('dev'));
...
var sessionManager	= new (require('express-sessions-cache'))({
	storage: 	'mongodb',
	host: 		options.mongo_server,
...
Gamify.sessionManager			= sessionManager;

var eSessionManager = require('express-sessionManager')(expressSession, Gamify);

app.use(expressSession({
	secret:	'gamify-passport',
	cookie:	{
		path:	'/',
...
app.use(expressCookieauth(Gamify));
...
var apiRoute = function(req, res) {
	...
	Gamify.log("req.body", req.body);
	Gamify.log("req.query", req.query);
